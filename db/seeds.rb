...
Application.create({ status: :applied, candidate_name: Faker::Name.name })

interview_application = Application.create({ status: :interview, candidate_name: Faker::Name.name })
interview = Interview.create({ interview_date: Time.zone.local(2024, 4, 30, 23, 59, 59),
...
hire = Hire.create({ hire_date: Time.zone.local(2024, 4, 30, 23, 59, 59), application: hire_application })
